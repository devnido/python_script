...
# cambiar este nombre por la tabla temporal en el datawarehouse para hacer la prueba de este script
# se debe crear la tabla primero en el datawarehouse (debe ser copia de fiscal_Comprob)
table_name = "fiscal_Comprob"

query = f"INSERT INTO DBO.{table_name} ({column_names}) VALUES ({placeholders})"

dwh_fiscal_columns = pd.read_sql(f"select top 1 * from [dbo].[{table_name}]", conn)
...
for filename in files:
    ...
    for chunk in pd.read_csv(filename, sep=';', decimal='.' ,chunksize=10000, encoding='latin1'):
        ...
        df = df.iloc[:,:26]
        fiscal_comprob = pd.concat([fiscal_comprob, df]) 

    missing_rows_fiscal_comprob = fiscal_comprob[~fiscal_comprob[['ComprobNum', 'StoreNum']].apply(tuple, axis=1).isin(dwh_fiscal[['ComprobNum', 'StoreNum']].apply(tuple, axis=1))]

    cursor = conn.cursor()

    missing_rows_fiscal_comprob.replace({np.inf: np.nan, -np.inf: np.nan}, inplace=True)
    missing_rows_fiscal_comprob = missing_rows_fiscal_comprob.fillna(0)
    ...
    values = missing_rows_fiscal_comprob.values.tolist()
